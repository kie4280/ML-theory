...
def show_reconstruction_faces(test_faces, test_num_file, eigenfaces):
    ...
    chosen = np.random.choice(test_num_file, 10, replace=False)
    for i in range(10):
        for eig in range(25):
...
def kernel(X:np.ndarray, mode:Literal["RBF", "Linear"]="RBF", gamma=0.0001) -> np.ndarray:
    """
    :param X: input data
    :param mode: the mode to construct the kernel
    :param (optional) gamma: the hyperparameter for the RBF kernel
    Return constructed kernel
    """
    if mode=="linear":
        K = X.T @ X
    elif mode=="RBF":
...
def pca(X:np.ndarray,num_dim:int|None=None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    :param X: the input data to do PCA
    :param num_dim: the number of dims to keep
...
def compute_eigen(K:np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
if __name__ == "__main__":
    train_X, train_filenames = load_faces("Training") 
    test_X, test_filenames = load_faces("Training") 
    ...
    K = kernel(train_X, mode="RBF")
    print(K.shape)
    M = row*col
    MM = np.zeros((M, M))/M
    cenK = K - MM.dot(K) - K.dot(MM) + MM.dot(K).dot(MM)

    ei_val, eigenfaces = compute_eigen(K)
    selected_eigenvector = eigenfaces[:, 0:25]
