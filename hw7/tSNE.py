...
def SNE(args, X:np.ndarray, labels=np.ndarray|None, max_iter = 1000):
    """
    ...
    _type = args.type  # {t-SNE|Symmetric SNE}
    perplexity = args.perplexity  # {5|20|35|50}
    no_dims = args.dims
    initial_dims = args.init_dims

    # Check inputs
    if isinstance(no_dims, float):
        RuntimeError("Error: array X should have type float.")
    if round(no_dims) != no_dims:
        RuntimeError("Error: number of dimensions should be an integer.")
    ...
    for iter in range(max_iter):
        ...
        # Visualize
        if (iter + 1) % 50 == 0 and labels is not None:
            visualize(args, Y, labels, str(iter + 1))
...
def visualize(args, Y:np.ndarray, labels:np.ndarray, _iter:str="Final"):
    fig = plt.figure()
    ...
    plt.axis('off')
    plt.savefig(f'{file_path}/{_type}/per_{perplexity}/{_iter}.jpg')
    if args.show_fig:
        plt.show()
    plt.close()

def plotSimilarity(args, P:np.ndarray, Q:np.ndarray, labels:np.ndarray):    
